            ...
            <div class="bg-primary-creamy overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-text-on-creamy">
                    <form method="POST" action="{{ route('reservations.store') }}">
                        ...
                        <!-- Reservation Date (Optional, could be set automatically) -->
                        <div class="mt-4">
                            <x-input-label for="reservation_date" :value="__('Reservation Date')" />
                            <x-text-input id="reservation_date" class="block mt-1 w-full" type="date" name="reservation_date" :value="old('reservation_date', \Carbon\Carbon::now()->format('Y-m-d'))" />
